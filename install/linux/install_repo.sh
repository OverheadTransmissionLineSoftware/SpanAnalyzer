...
# This script copies files from the repo and installs to the local machine.
...
mkdir -p $DIR_APP
cp build/codeblocks/release/SpanAnalyzer $DIR_APP

# copies resource files
DIR_RESOURCES=$DIR_APP/res
mkdir -p $DIR_RESOURCES
cp res/help/*.htb $DIR_RESOURCES

# copies example file(s)
DIR_EXAMPLES=$DIR_APP/examples
mkdir -p $DIR_EXAMPLES
cp test/cables/*.cable $DIR_EXAMPLES
cp test/*.spananalyzer $DIR_EXAMPLES
